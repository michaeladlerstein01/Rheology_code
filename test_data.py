...
if __name__ == "__main__":       
    ...
    test_area =  200*torch.sin(x)**2 + 500 
# Triangle wave between 0 and 1
    triangle = 2 * torch.abs(2 * (x / (2*torch.pi) - torch.floor(x / (2*torch.pi) + 0.5)))  

    # Scale to match your previous amplitude and offset
    test_area = 500 * triangle      

    plt.plot(x , test_area)
    plt.show()
    ...
    with torch.no_grad():
        c_sin_B[:, 0] = 0.0
    optimizer = torch.optim.Adam([c_cos_B, c_sin_B], lr=0.01)
    ...
    for epoch in range(10000):
        ...
        area_pred = area_sweep_torch(fixed_plate, pattern_B)
        ...
        rad_loss = radial_edge_unimodality_loss(pattern_B , radius_px= radius_px , num_angles= 72)
        ...
        if epoch % 2000 == 0:
            ...
            fixed_plate_plt = fixed_plate.detach().cpu().numpy()
            ...
            ax1.imshow(fixed_plate_plt, cmap='gray', origin='lower')
